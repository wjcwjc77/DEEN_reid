...
from cmt import LocalPerceptionUint, InvertedResidualFeedForward, make_pairs
...
class Gated_module(nn.Module):
    def __init__(self, channel):
        super(Gated_module, self).__init__()

        # 开始时用于放大特征图的转置卷积层
        self.upsample_f1_gate = nn.ConvTranspose2d(channel, channel * 2, kernel_size=2, stride=2, padding=0)
        self.upsample_f = nn.ConvTranspose2d(channel, channel * 2, kernel_size=2, stride=2, padding=0)
        self.upsample_fi_gate = nn.ConvTranspose2d(channel, channel * 2, kernel_size=2, stride=2, padding=0)
        self.downsample_f1 = nn.Conv2d(channel * 2, channel, kernel_size=3, stride=2, padding=1)
        self.downsample_fi = nn.Conv2d(channel * 2, channel, kernel_size=3, stride=2, padding=1)

        self.upsample_f1_gate.apply(weights_init_kaiming)
        self.upsample_f.apply(weights_init_kaiming)
        self.upsample_fi_gate.apply(weights_init_kaiming)
        self.downsample_f1.apply(weights_init_kaiming)
        self.downsample_fi.apply(weights_init_kaiming)
    ...
    def forward(self, x):
        ...
        f1_gate = self.upsample_f1_gate(x)
        fi_gate = self.upsample_fi_gate(x)
        x = self.upsample_f(x)

        f1 = self.downsample_f1(x * self.silu(f1_gate))
        fi = self.downsample_fi(x * self.silu(fi_gate))

        out = torch.cat((x_ori, f1, fi), 0)
...
class ConvDW3x3(nn.Module):
    def __init__(self, dim, padding, dilation):
        super(ConvDW3x3, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=padding, bias=False, dilation=dilation),
            nn.GELU(),
        )
...
class MY_DEE(nn.Module):
    def __init__(self, channel):
        super(MY_DEE, self).__init__()
        self.in_channel = channel
        self.out_channel = channel // 4
        self.dropout = nn.Dropout(p=0.01)
        self.conv1x1_gelu_1 = nn.Sequential(
        ...
        self.conv1x1_gelu_2 = nn.Sequential(
            nn.Conv2d(in_channels=self.in_channel,
                      out_channels=self.out_channel,
                      kernel_size=1,
                      stride=1,
        ...
        self.conv3x3_dw11 = ConvDW3x3(dim=self.out_channel, padding=1, dilation=1)
        self.conv3x3_dw12 = ConvDW3x3(dim=self.out_channel, padding=2, dilation=2)
        self.conv3x3_dw13 = ConvDW3x3(dim=self.out_channel, padding=3, dilation=3)

        self.conv3x3_dw21 = ConvDW3x3(dim=self.out_channel, padding=1, dilation=1)
        self.conv3x3_dw22 = ConvDW3x3(dim=self.out_channel, padding=2, dilation=2)
        self.conv3x3_dw23 = ConvDW3x3(dim=self.out_channel, padding=3, dilation=3)

        self.conv1x1_pw_1 = nn.Sequential(
            nn.Conv2d(self.out_channel, channel, 1, 1, 0),
        )
        self.conv1x1_pw_2 = nn.Sequential(
            nn.Conv2d(self.out_channel, channel, 1, 1, 0),
        )
        # reZero
        self.alpha11 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha12 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha13 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha21 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha22 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha23 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数

    def forward(self, x):
        x1 = self.conv1x1_gelu_1(x)
        x1 = (x1 + self.alpha11 * self.conv3x3_dw11(x1)) + (x1 + self.alpha12 * self.conv3x3_dw12(x1)) + (
                    x1 + self.alpha13 * self.conv3x3_dw13(x1))
        x1 = self.conv1x1_pw_1(x1)

        x2 = self.conv1x1_gelu_2(x)
        x2 = (x2 + self.alpha21 * self.conv3x3_dw21(x2)) + (x2 + self.alpha22 * self.conv3x3_dw22(x2)) + (
                    x2 + self.alpha23 * self.conv3x3_dw23(x2))
        x2 = self.conv1x1_pw_2(x2)
...
class MFA_block(nn.Module):
    def __init__(self, high_dim, low_dim, flag, ffn_ratio=4., drop_path_rate=0.):
        ...
        self.alpha1 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha2 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数
        self.alpha3 = nn.Parameter(torch.zeros(1))  # 添加ReZero可学习参数

        self.att = CNL(high_dim, low_dim, flag)
        # self.PNL = PNL(high_dim, low_dim)

    def forward(self, x, x0):
        lpu_x = self.LPU_high(x)

        x = x + self.alpha1 * lpu_x

        lpu_x0 = self.LPU_low(x0)  # 这里x0的 channel数为（64），所以要用不同的LPU

        x0 = x0 + self.alpha2 * lpu_x0
        ...
        z = z + self.alpha3 * self.drop_path(ffn)
        return z
...
class embed_net(nn.Module):
    def __init__(self, args, class_num, dataset, arch='resnet50'):
        ...
        if self.dataset == 'regdb':  # For regdb dataset, we remove the MFA3 block and layer4.
            pool_dim = 1024
            # self.GEE = Gated_module(512)
            self.DEE = MY_DEE(512)
            self.MFA1 = MFA_block(256, 64, 0)
            self.MFA2 = MFA_block(512, 256, 1)
        else:
            pool_dim = 2048
            # self.GEE = Gated_module(1024)
            self.DEE = MY_DEE(1024)
            self.MFA1 = MFA_block(256, 64, 0)
            self.MFA2 = MFA_block(512, 256, 1)
